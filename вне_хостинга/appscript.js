// ============================================
// КОД ДЛЯ ТАБЛИЦЫ ПАРТНЁРА ФРАНКЛИНС
// ============================================

// ID вашей Google Таблицы
var SPREADSHEET_ID = '1ZsMf7uaHtftOVlSZA7_kh1VtlXaQ0ogU0ZnsCj4-aBE';

// Названия листов
var PAYMENTS_SHEET_NAME = 'Выплаты';
var DOCUMENTS_SHEET_NAME = 'Документы';
var DETOURS_SHEET_NAME = 'Объезды';
var PAYMENTS_MIN_YEAR = 2026;
...
function getPaymentYear_(row, columnIndexes) {
  if (columnIndexes.year >= 0) {
    var year = parseInt(row[columnIndexes.year], 10);
    if (!isNaN(year) && year >= 2000) return year;
  }

  if (columnIndexes.period >= 0) {
    var period = String(row[columnIndexes.period] || '');
